...
export const ProductTableRow: React.FC<ProductTableRowProps> = ({
  ...
  function getAttributeValue(slugName: string): any {
    const matchingAttribute = product?.attributes?.filter(({ attribute: { slug } }) => slug === slugName);
  ...
  const manufacturer =
    manufacturers.find(({ mcode, manufacturer }) => mcode === productMcode)?.manufacturer || productMcode;
  console.log(product);
  return (
    <tr>
      <td className="pr-0">
      ...
      <td>
        ...
        <Link style={{ textDecoration: 'underline' }} to={`/products/${product?.slug}`}>
          {product?.name}
        </Link>
        <div className="small mt-1">
          Spec Code: {getMetadataValue('mpn')} | Ordering Code:{' '}
          {product?.attributes && getAttributeValue('ordering-code')}
        </div>
      </td>
